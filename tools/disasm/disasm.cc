...
std::string get_first_word(const std::string& insn) {
    std::stringstream ss(insn);
    std::string first_word;
    ss >> first_word;
    return first_word;
}

std::vector<std::string> scala_need_frm = {
  "fadd.s", "fsub.s", "fadd.d", "fsub.d",
  "fcvt.w.s", "fcvt.wu.s", "fcvt.l.s", "fcvt.lu.s",
  "fcvt.w.d", "fcvt.wu.d", "fcvt.l.d", "fcvt.lu.d", "fcvt.s.d", "fcvt.d.s",
  "fround.h", "fround.s", "fround.d", "froundnx.h", "froundnx.s", "froundnx.d",
  // i2f
...
bool find_str_vector(const std::string& str, const std::vector<std::string>& strVec) {
...
std::string disassembler_t::disassemble(insn_t insn) const
{
  ...
  std::string insn_str = disasm_insn->to_string(insn);

  // check
  std::string insn_name = get_first_word(insn_str);
  uint64_t rm = BITS(insn.bits(), 14, 12);
  if (find_str_vector(insn_name, scala_need_frm) && (rm == 5 || rm == 6)) {
    return "unknown";
  }
...
std::string disassembler_t::disassemble_custom_insn(insn_t insn, uint64_t opcode) const
{
  if (BITS(insn.bits(), 6, 0) != opcode) {
    return "unknown";
  }
