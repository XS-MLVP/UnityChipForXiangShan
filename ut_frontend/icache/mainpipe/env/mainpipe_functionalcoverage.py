...
def define_mainpipe_coverage(bundle, dut):
    ...
    MainPipe_dict = {
        ...
        # S2 stage control signals (用于观测时序)
    ...
    g.add_watch_point(
        ...
        bins={
            ...
            # 12.5: 多路命中（通道1，命中数>=2且meta corrupt）
            "CP12.5_multi_way_hit_1": lambda d: d["ecc_enable"].value == 1 and \
                                               d["s2_meta_corrupt_1"].value == 1 and \
                                               d["s1_meta_corrupt_hit_num_1"].value >= 2 and \
                                               d["errors_1_valid"].value == 1 and \
                                               d["errors_1_report_to_beu"].value == 1,
            
            # 12.6: ECC功能关闭（meta corrupt始终为0）
    ...
    g.add_watch_point(
        ...
        bins={
            ...
            "CP13.2_channel0_pmp_exception": lambda d: d["pmp_0_resp_instr"].value == 0 and \
                                                      d["fetch_resp_exception_0"].value != 0 and \
                                                      d["fetch_resp_valid"].value == 1,
            
            # 13.3: 通道1有PMP异常（PMP响应异常且最终有异常输出）
            "CP13.3_channel1_pmp_exception": lambda d: d["pmp_1_resp_instr"].value == 0 and \
                                                      d["fetch_resp_exception_1"].value != 0 and \
                                                      d["fetch_resp_valid"].value == 1,
    ...
    g.add_watch_point(
        ...
        bins={
            ...
            # 16.3: 多Bank ECC错误 - 修复：删除不存在的source.data检查
    ...
    g.add_watch_point(
        {
            ...
            # S2阶段状态和请求信息
            "s2_valid": bundle.s2._valid,
            "s2_req_vaddr_0": bundle.s2._req._vaddr._0,
            "s2_req_vaddr_1": bundle.s2._req._vaddr._1, 
            "s2_req_ptags_0": bundle.s2._req._ptags._0,
            "s2_req_ptags_1": bundle.s2._req._ptags._1,
            "s2_doubleline": bundle.s2._doubleline,
            "s1_fire": bundle.s1._fire,
            
            # 数据更新监控信号
            "s2_hits_0": bundle.s2._hits._0,
            "s2_hits_1": bundle.s2._hits._1,
            "s2_data_is_from_MSHR_0": bundle.s2._data._is._from._MSHR._0,
            "s2_data_is_from_MSHR_1": bundle.s2._data._is._from._MSHR._1,
        },
        bins={
            ...
            "CP18.1_mshr_hit_single_line": lambda d: (
                ...
                d["s2_req_vaddr_0"].value[13:6] == d["mshr_resp_vSetIdx"].value and
                d["s2_req_ptags_0"].value == d["mshr_resp_blkPaddr"].value[41:6]
            ),
            ...
            "CP18.1_mshr_hit_double_line": lambda d: (
                ...
                (
                    ...
                    or
                    # 端口1匹配
                    (d["s2_req_vaddr_1"].value[13:6] == d["mshr_resp_vSetIdx"].value and
                     d["s2_req_ptags_1"].value == d["mshr_resp_blkPaddr"].value[41:6])
                )
            ),
