...
async def test_2_2_1(predchecker_env):
    print("Test 1_2_1")
    sqr = pred_checker_sqr()
    mdl = pred_checker_mdl()
    res = []
    ref_res = []
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, 100000, 7)
...
@toffee_test.testcase
async def test_2_2_2(predchecker_env):
    print("Test 1_2_2")
    sqr = pred_checker_sqr()
    mdl = pred_checker_mdl()
    res = []
    ref_res = []
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, 100000, 8)
    #print(f"vec_pkt: \n{vec_pkt}\n")
    for i in range(len(vec_pkt)):
        print(f"Test cycle {i}")
        async for res in predchecker_env.predCheckerAgent.agent_pred_check(*vec_pkt[i]):
