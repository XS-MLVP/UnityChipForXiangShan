...
async def test_ret_2_1_1_single(predchecker_env):
    print("Test 2.1.1(single)")
...
@toffee_test.testcase 
async def test_ret_2_1_1(predchecker_env):
    print("Test 2.1.1")
    res = []
    ref_res = []
    mdl = pred_checker_mdl()
    sqr = pred_checker_sqr()
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, 1000, 5)
...
@toffee_test.testcase
async def test_2_1_2(predchecker_env):
    print("Test 2_1_2")
    sqr = pred_checker_sqr()
    mdl = pred_checker_mdl()
    res = []
    ref_res = []
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, 100000, 6)
    for i in range(len(vec_pkt)):
        print(f"Test cycle {i}")
        async for res in predchecker_env.predCheckerAgent.agent_pred_check(*vec_pkt[i]):
            if len(res) == 2:
