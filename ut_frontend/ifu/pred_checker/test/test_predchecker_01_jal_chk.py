...
async def test_jal_chk_1_1_1(predchecker_env):
    print("Test case 1.1.1")
    res = []
    ref_res = []
    mdl = pred_checker_mdl()
    sqr = pred_checker_sqr()
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, TEST_CYCLES, 1)
...
async def test_jal_chk_1_2_1(predchecker_env):
    ...
    res = []
    ref_res = []
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, TEST_CYCLES, 3)
    for i in range(len(vec_pkt)):
        print(f"Test cycle {i}")
        async for res in predchecker_env.predCheckerAgent.agent_pred_check(*vec_pkt[i]):
            if len(res) == 2:
...
async def test_jal_chk_1_2_2(predchecker_env):
    ...
    res = []
    ref_res = []
    vec_pkt = sqr.gen_vec(PREDICT_WIDTH, TEST_CYCLES, 4)
    #print(f"vec_pkt: \n{vec_pkt}\n")
    for i in range(len(vec_pkt)):
        print(f"Test cycle {i}")
        async for res in predchecker_env.predCheckerAgent.agent_pred_check(*vec_pkt[i]):
