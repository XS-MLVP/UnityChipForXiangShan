...
async def test_chain2_match(frontend_trigger_env: FrontendTriggerEnv):
    ...
    chained_bp_update_gen_list = chained_bp_update_generator(
        pc_width=50, chain_count=2, condition=lambda x: x.select == 0
    )
...
async def test_chain3_match(frontend_trigger_env: FrontendTriggerEnv):
    ...
    chained_bp_update_gen_list = chained_bp_update_generator(
        pc_width=50, chain_count=3, condition=lambda x: x.select == 0
    )
...
async def test_chain4_match(frontend_trigger_env: FrontendTriggerEnv):
    ...
    chained_bp_update_gen_list = chained_bp_update_generator(
        pc_width=50, chain_count=4, condition=lambda x: x.select == 0
    )
