...
def construct_instrs_with_jump_idx(end_idx, cfi_res=-1, imm=-1, last_finished=True, invalid_jmp_prediction=False):
    """
        params
        - end_idx: the last idx of the seq, either end of full 'instrs' or jump idx
        - cfi_res: the given instr type, if -1, will select one from (1, 3)
        - imm: jump target to gen instr
        - last_finished: if the prediction block will end, or leaving the res to next prediction block
        - invalid_jmp_prediction: decide if the end idx is a valid instr(will be override by last finished check)
        
        returns:
        - 17 16-bit raw instrs
    """
    instrs = [ 0 for _ in range(PREDICT_WIDTH+1)]
    seq_end_idx = max(0, min(end_idx, PREDICT_WIDTH-1))
    for i in range(seq_end_idx):
        # here we have to ensure that the last instr before jump/15th instr is ended correctly
        # or if we want to detect invalid prediction, we have to let jump instr to be higher place of 32bit instr
        if i < seq_end_idx - 2:
            is_rvc = randbool()
        elif i == seq_end_idx - 2:
            is_rvc = invalid_jmp_prediction and seq_end_idx != PREDICT_WIDTH -1 
        else:
            # can not influence last finished
            is_rvc = (not invalid_jmp_prediction) or seq_end_idx == PREDICT_WIDTH - 1
    ...
    for i in range(seq_end_idx, PREDICT_WIDTH+1):
        if i == seq_end_idx:
        ...
        # here we ensures cross-prediction-block instr selection
        if i == PREDICT_WIDTH - 1:
            if last_finished:
                is_rvc = True
            else:
                is_rvc = False
