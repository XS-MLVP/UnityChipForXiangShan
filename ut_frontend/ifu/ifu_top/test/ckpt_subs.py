...
def is_provider_jmp_off_type(pos, rvc_val, j_type):
    ...
    def provider_jmp_off_type(env: IFUTopEnv):
        return env.top_agent.top.internal_wires.pre_decoder.out_pd_isRVCs[pos].value == rvc_val \
            and env.top_agent.top.internal_wires.f3_predecoder.out_pd_brTypes[pos].value == j_type
...
def is_provider_cfi_type(pos, cfi_type):
    ...
    assert 0 <=cfi_type <= 3
    def provider_jmp_off_type(env: IFUTopEnv):
        return env.top_agent.top.internal_wires.f3_predecoder.out_pd_brTypes[pos].value == cfi_type
    return provider_jmp_off_type

def is_provider_jal_ret(pos, rvc, jal_ret_type):
    assert 0 <= pos < PREDICT_WIDTH
    def provider_jal_ret(env: IFUTopEnv) -> bool:
        is_ret = env.top_agent.top.internal_wires.f3_predecoder.out_pd_isRets[pos].value
        is_call = env.top_agent.top.internal_wires.f3_predecoder.out_pd_isCalls[pos].value
