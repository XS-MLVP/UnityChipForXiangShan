...
def is_provider_icache_ready(val):
    def provider_icache_ready(env: IFUTopEnv):
        return env.top_agent.top._icacheInterCtrl._icacheInter._icacheReady.value == val
    return provider_icache_ready
...
def is_provider_icache_all_valid(val):
    def provider_icahce_all_valid(env: IFUTopEnv):
        return env.top_agent.top.internal_wires._icacheRespAllValid.value == val
    return provider_icahce_all_valid
...
def get_coverage_group_tops(env: IFUTopEnv):
    ...
    group.add_watch_point(env, {
        f"_{bool(i)}": is_provider_icache_ready(i) for i in range(2)
    }, name="icache_ready")
    
    group.add_watch_point(env, {
        f"_{bool(i)}": is_provider_icache_all_valid(i) for i in range(2)
    }, name="icache_all_valid")
