...
from ..datadef import FTQQuery, ICacheStatusResp, FTQFlushInfo, FTQResp, FrontendTriggerReq, \
        ...
        PreDecodeDataDef, F3PreDecodeData, PredCheckerRetData, PredCheckerStage1RetData, PredCheckerStage2RetData, FTQRedirect, \
            FTQFlushFromBPU, NonMMIOSingleReq, NonMMIOResp, MMIOCycleInfo, MMIOReq, MMIOState, MMIOToIbufferFTQ, mmio_data_logger_instance,\
                MMIOStateDataLogger, non_mmio_logger_instance, NonMMIOReqLogger, ClusteredNonMMIOReqs, ICacheResp

class HistoryStat():
    def __init__(self,
                 cluster: ClusteredNonMMIOReqs,
                 resps: list[NonMMIOResp],
                 tasks: list,
                 valids: str
                 ):
...
class OutsideAgent(Agent):
    ...
    def get_readys(self):
        return {1:self.top.internal_wires._f1_ready.value, 2: self.top.internal_wires._f2_ready.value, 3: self.top.internal_wires._f3_ready.value}
    
    def get_valids(self):
        return {0:self.top.io_ftqInter._fromFtq._req._ready.value, 1:self.top.internal_wires._f1_valid.value, \
            2: self.top.internal_wires._f2_valid.value, 3: self.top.internal_wires._f3_valid.value}
    ...
    def get_f3predecoder_res(self)->F3PreDecodeData:
        ...
        f3predecode_data.brTypes = [br_type.value for br_type in self.top.internal_wires.f3_predecoder.out_pd_brTypes]
        f3predecode_data.isCalls = [is_call.value for is_call in self.top.internal_wires.f3_predecoder.out_pd_isCalls]
        f3predecode_data.isRets = [is_ret.value for is_ret in self.top.internal_wires.f3_predecoder.out_pd_isRets]
    ...
    # 实际上，核心的指令“数据”在这个周期才给出
    def fake_resp_stat(self, icacheStatusResp: ICacheStatusResp):
        icache_resp = icacheStatusResp.resp
        self.fake_resp(icache_resp)
    
    def fake_resp(self, icache_resp: ICacheResp):
    ...
    @driver_method()
    async def deal_with_non_mmio_clusters(self, reqs: ClusteredNonMMIOReqs):
        """处理一组非MMIO的数据请求

        Args:
            reqs (ClusteredNonMMIOReqs): 非MMIO数据请求

        Returns:
            计算结果: （返回值目前用不到）
        """
        tasks : list[list] = []
        req_cnt = len(reqs.reqs)
        resps = [NonMMIOResp() for _ in range(req_cnt)]
        next_appear = 0
        valids_stats = ""
        
        def get_stage0_inputs_item(ftq_query: FTQQuery, flush_from_bpu: FTQFlushFromBPU, icache_ready:bool):
            def stage0_inputs():
                self.query_from_ftq(ftq_query)
                self.from_ftq_flush(flush_from_bpu)
                self.set_icache_ready(icache_ready)
            return stage0_inputs
        
        def get_stage2_inputs(icache_resp: ICacheResp):
            def stage2_inputs():
                self.fake_resp(icache_resp)
            return stage2_inputs
        
        def get_stage3_inputs(fs_is_off: bool):
            def stage3_inputs():
                self.set_fs_is_off(fs_is_off)
                self.set_ibuffer_ready(True)
            return stage3_inputs
    
        def get_collect_stage0_res(resp: NonMMIOResp):
            def collect_stage0_res():
                resp.ftq_ready = self.get_ftq_ready()
                resp.bpu_flush_res = self.get_bpu_flush()
            return collect_stage0_res

        def get_collect_stage2_res(resp: NonMMIOResp):
            def collect_stage2_res():
                if (not resp.ftq_ready) or resp.bpu_flush_res:
                    # f0 is failing, so ignore
                    return
                ...
                resp.exceptions = self.get_exception()
                assert resp.icache_all_valid != 0, f"HISTORY:\n\n{self.replay_stats_stack}\nTASKS: {tasks}\ncluster:{reqs}\nVALIDS:{valids_stats}\ncur_resp:{resp}"
            return collect_stage2_res
            
        def get_check_f2_failed():
            def check_f2_failed():
                all_valid = self.get_icache_all_resp()
                # 这里只需要检查即可
                assert all_valid == 0, f"TASKS: {tasks}\ncluster:{reqs}\nVALIDS:{valids_stats}\n"
            return check_f2_failed
        
        def get_collect_stage3_res(resp: NonMMIOResp):
            def collect_stage3_res():
                if (not resp.ftq_ready) or resp.bpu_flush_res:
                    # f0 is failing, so ignore
                    return
                resp.exception_vecs = self.get_exception_vecs()
            ...
            return collect_stage3_res
        
        def get_collect_stage_wb_res(resp: NonMMIOResp):
            def collect_stage_wb_res():
            ...
            return collect_stage_wb_res
        next_f2_appear = 2
        
        # 正常安排
        NON_MMIO_TASK_STATE=0
        
        for i in range(req_cnt):
            req = reqs.reqs[i]
            ref = reqs.resps[i]
            resp = resps[i]
            cur_appear = next_appear
            tasks_limit = next_f2_appear + req.get_icache_block_terms() + 4
            # print(f"task limit: {tasks_limit}")
            # 这里+1是考虑到收集wb输出的周期要在输入周期的后一个周期
            if tasks_limit > len(tasks):
                # 补足到下一个出现者之前
                for _ in range(len(tasks), tasks_limit):
                    tasks.append([])
            tasks[cur_appear].append(get_stage0_inputs_item(req.ftq_req, req.bpu_flush_info, req.icache_ready))
            tasks[cur_appear+1].append(get_collect_stage0_res(resp))
            cur_appear = next_f2_appear
            # 这里跳过了MMIO阻塞的周期
            for j in range(req.get_icache_block_terms()):
                tasks[cur_appear].append(get_stage2_inputs(req.invalid_icache_resps[j]))
                cur_appear+=1
                tasks[cur_appear].append(get_check_f2_failed())
            
            # set stage 2 input 
            tasks[cur_appear].append(get_stage2_inputs(req.final_icache_resp))
            cur_appear+=1
            if NON_MMIO_TASK_STATE == 0 and ref.pin_flush:
                NON_MMIO_TASK_STATE = 1
                next_appear += 1
                next_f2_appear = cur_appear
            elif NON_MMIO_TASK_STATE == 1:
                # 这个状态下，前一个请求的pin_flush为真，也就是前一个请求冲刷
                if ref.f3_flush:
                # 如果这个请求是个新请求，则在这个请求的“收集结果”阶段的上一个阶段，放入下一个新请求
                # 也就是下个请求的开始属于二类请求
                    NON_MMIO_TASK_STATE = 0
                    # 当前请求执行collect stage3时，就需要放入下一个请求
                    next_appear = next_f2_appear + req.get_icache_block_terms() + 2
                else:
                    NON_MMIO_TASK_STATE = 0
                    next_appear = next_f2_appear + req.get_icache_block_terms() + 3
                next_f2_appear = next_appear + 2
                
            else:
                NON_MMIO_TASK_STATE = 0
                next_appear += 1
                next_f2_appear = cur_appear
            tasks[cur_appear].append(get_collect_stage2_res(resp))
            
            tasks[cur_appear].append(get_stage3_inputs(req.fs_is_off))
            cur_appear+=1
            tasks[cur_appear].append(get_collect_stage3_res(resp))
            
            tasks[cur_appear+1].append(get_collect_stage_wb_res(resp))                       

        # 此时获得的tasks_limit就是整个过程执行的周期数，那么需要step这么多次
        for i in range(tasks_limit):
            valids_stats = f"{valids_stats}valids:{self.get_valids()}\n"
            for task in tasks[i]:
                task()
            # if i < next_appear - 1:
            await self.step()
        if len(self.replay_stats_stack) > 5:
            self.replay_stats_stack.pop(0)
        self.replay_stats_stack.append(HistoryStat(reqs, resps, tasks, valids_stats))
        assert resps == reqs.resps, f"DUTOUT:\n{resps};\n\n\n\n\n STDOUT:\n{reqs.resps}\n\n\n\n\n HISTORY:\n{self.replay_stats_stack}"        
        return
            
    
    @driver_method()
    async def set_up_before_mmio_states(self, mmio_cycle_info: MMIOCycleInfo):
        mmio_data_logger_instance.start_new_reqs(mmio_cycle_info)
        ftq_query = FTQQuery()
        ...
        self.fake_resp_stat(icache_resp)
    ...
    @driver_method()
    async def cmp_single_mmio_req(self, mmio_req: MMIOReq, ref):
        res = await self.deal_with_single_mmio_req(mmio_req)
        assert res == ref, f"HISTORY:\n {mmio_data_logger_instance.history_reqs}\n\n\n\n\n\n \
    ...
    async def deal_with_single_mmio_req(self, mmio_req:MMIOReq):
        ...
        Args:
            mmio_req (MMIOReq): one single mmio req that may cause state transfer

        Returns:
            current state and some more infos
        """
        mmio_data_logger_instance.add_new_req(mmio_req)
