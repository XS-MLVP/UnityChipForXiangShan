...
def build(cfg):
    ...
    if not os.path.exists(get_root_dir("dut/TLBuffer")):
        ...
        s, _, _ = exe_cmd(f'picker export --cp_lib false {get_rtl_dir("rtl/TLBuffer.sv", cfg=cfg)} --fs {get_root_dir("scripts/frontend_itlb/tlbuffer_rtl_files.f")} --lang python --tdir {get_root_dir("dut")}/ -w TLBuffer.fst -c')
        assert s, "Failed to export TLBuffer.sv"
    # build disasm
