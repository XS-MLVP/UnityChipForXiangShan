...
def build(cfg):
    ...
    if not match_version(cfg.rtl.version, "openxiangshan-kmh-*"):
        warning("frontend_tlb_storage_wrapper: %s" % f"Unsupported RTL version {cfg.rtl.version}")
        return False
    # check files
    f = is_all_file_exist(["rtl/TlbStorageWrapper.sv"], get_rtl_dir(cfg=cfg))
    assert f is True, f"File {f} not found"
    # find source files for TlbStorageWrapper
    rtl_files = get_all_rtl_files("TlbStorageWrapper", cfg=cfg)
    info(f"rtl_files: {rtl_files}")
    assert rtl_files, "Cannot find RTL files of TlbStorageWrapper"

    # build
    # export TlbStorageWrapper.sv
    if not os.path.exists(get_root_dir("dut/TlbStorageWrapper")):
        info("Exporting TlbStorageWrapper.sv")
        with NamedTemporaryFile("w+", encoding="utf-8", suffix=".txt") as filelist:
            ...
            s, _, err = exe_cmd(
                ...
                f"{get_root_dir('dut')}/ -w TlbStorageWrapper.fst -c")
        assert s, err
    return True
