...
def build(cfg):
    ...
    if not match_version(cfg.rtl.version, "openxiangshan-kmh-*"):
        error(f"frontend_icache_top: Unsupported RTL version {cfg.rtl.version}")
        return False
    
    # find source files for ICacheTop
    ...
    # internal_signals_path = os.path.join(get_root_dir("scripts/icache_related/icache_top_internals.yaml"))
