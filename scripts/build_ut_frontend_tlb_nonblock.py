...
def build(cfg):
    ...
    if not os.path.exists(get_root_dir("dut/TLBNonBlock")):
        ...
        s, _, _ = exe_cmd(f'picker export --cp_lib false {get_rtl_dir("rtl/TLBNonBlock.sv", cfg=cfg)} --fs {get_root_dir("scripts/frontend_itlb/tlb_nonblock_rtl_files.f")} --lang python --tdir {get_root_dir("dut")}/ -w TLBNonBlock.fst -c')
        assert s, "Failed to export TLBNonBlock.sv"
    # build disasm
