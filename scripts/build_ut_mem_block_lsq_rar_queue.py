...
def build(cfg):
    ...
    if not os.path.exists(get_root_dir("dut/LoadQueueRAR")):
        ...
        rtl_files = get_all_rtl_files("LoadQueueRAR", cfg=cfg)
        internal_signals_path = os.path.join(get_root_dir("scripts/mem_block_lsq_rar_queue/internal.yaml"))
        ...
        with NamedTemporaryFile("w+", encoding="utf-8", suffix=".txt") as filelist:
            filelist.write("\n".join(rtl_files))
            filelist.flush()
            s, out, err = exe_cmd(f'picker export --cp_lib false {get_rtl_dir("rtl/LoadQueueRAR.sv", cfg=cfg)} --fs {filelist.name} --lang python --tdir {get_root_dir("dut")}/ -w RAR.fst --internal={internal_signals_path} -c')
        assert s, "Failed to export LoadQueueRAR.sv: %s\n%s" % (out, err)
    return True

def get_metadata():
    return {
        "dut_name": "mem_block_lsq_rar_queue",
        "dut_dir": "LoadQueueRAR",
        "test_targets": [
            "ut_mem_block/lsq/rar_queue",
            "ut_mem_block/lsq/",
            "ut_mem_block/"
        ]
    }
