...
async def _wait_ptw_req_and_capture(env, vaddr, max_cycles=32):
    vpn_expect = (int(vaddr) >> 12) & ((1 << 27) - 1)
    for _ in range(max_cycles):
        for i in range(4):
            req = env.bundle.ptw.req[i]
...
async def do_sfence(env, *, hv=False, hg=False, rs1=False, addr=0, rs2=False, id_=0, settle_cycles=10):
    sf = env.bundle.sfence
    ...
    sf.bits_id.value   = int(id_) & ((1 << 16) - 1)   # VMA: ASID；GVMA: VMID；VVMA: 实际忽略
...
async def test_sfence_vma_global_noG(dtlb_env):
    env = dtlb_env
    env.dut.reset.value = 1; await env.bundle.step()
    env.dut.reset.value = 0; await env.bundle.step()
    await env.set_sv39_defaults(); await env.bundle.step()
    ...
    # miss → 回填（非 G）→ hit
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD, return_on_miss=True) is None
    _, s2x, _ = await _wait_ptw_req_and_capture(env, va)
    await env.req.set_ptw_resp(vaddr=va, paddr=pa, level=0,
                               s1_asid=asid, s1_perm_r=True, s1_perm_a=True, s1_perm_g=False,
                               s2xlate=s2x)
    expect = (pa & ~0xFFF) | (va & 0xFFF)
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD) == expect

    # 全量清（rs1=0, rs2=0）
    await do_sfence(env, hv=False, hg=False, rs1=False, rs2=False)
...
async def test_sfence_vma_by_asid(dtlb_env):
    ...
    await env.req.set_ptw_resp(vaddr=va, paddr=pa, level=0,
                               s1_asid=asid, s1_perm_r=True, s1_perm_a=True, s1_perm_g=False,
                               s2xlate=s2x)
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD) == ((pa & ~0xFFF) | (va & 0xFFF))

    # 按当前 satp.asid 清（id 用 CSR.Satp.asid）
    await do_sfence(env, hv=False, hg=False, rs1=False, rs2=True, id_=asid)
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD, return_on_miss=True) is None
...
async def test_sfence_vma_by_va(dtlb_env):
    ...
    await do_sfence(env, hv=False, hg=False, rs1=True, addr=va, rs2=False)
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD, return_on_miss=True) is None
...
async def test_sfence_vma_by_va_asid(dtlb_env):
    ...
    await do_sfence(env, hv=False, hg=False, rs1=True, addr=va, rs2=True, id_=asid)
    assert await env.req.drive_request(port=port, vaddr=va, cmd=LOAD, return_on_miss=True) is None
...
async def test_hfence_vvma_by_vmid(dtlb_env):
    ...
    assert await env.req.drive_request(port=port, vaddr=gva, cmd=LOAD, return_on_miss=True) is None
    _, s2x, _ = await _wait_ptw_req_and_capture(env, gva)
    await env.req.set_ptw_resp(vaddr=gva, paddr=gpa, level=0,
                               s1_asid=asid, s1_vmid=vmid,
                               s1_perm_r=True, s1_perm_a=True, s1_perm_u=True, s1_perm_g=False,
                               s2xlate=s2x)
    assert await env.req.drive_request(port=port, vaddr=gva, cmd=LOAD) == ((gpa & ~0xFFF) | (gva & 0xFFF))

    # VVMA：hv=1，通常按 VMID 过滤需 rs2=1；VMID 来源于 CSR.HGATP.vmid（id 忽略）
    await do_sfence(env, hv=True, hg=False, rs1=False, rs2=True, id_=vmid)
    assert await env.req.drive_request(port=port, vaddr=gva, cmd=LOAD, return_on_miss=True) is None
...
async def test_hfence_vvma_by_gva_vmid(dtlb_env):
    ...
    await do_sfence(env, hv=True, hg=False, rs1=True, addr=gva, rs2=True, id_=vmid)
    assert await env.req.drive_request(port=port, vaddr=gva, cmd=LOAD, return_on_miss=True) is None
...
async def test_hfence_gvma_by_vmid(dtlb_env):
    ...
    assert await env.req.drive_request(port=port, vaddr=gpa_in, cmd=LOAD, return_on_miss=True) is None
    _, s2x, getGpa_req = await _wait_ptw_req_and_capture(env, gpa_in)
    await env.req.set_ptw_resp(vaddr=gpa_in, paddr=0, level=0, s1_v=False,
                               s2xlate=s2x, getGpa=getGpa_req,
                               s2_tag=(gpa_in >> 12) & ((1<<27)-1),
                               s2_ppn=(hpa >> 12), s2_level=0,
                               s1_vmid=vmid, s2_perm_r=True, s2_perm_a=True)
    assert await env.req.drive_request(port=port, vaddr=gpa_in, cmd=LOAD) == ((hpa & ~0xFFF) | (gpa_in & 0xFFF))

    # GVMA：hg=1，按 VMID（来自 bits_id）清 S2
    await do_sfence(env, hv=False, hg=True, rs1=False, rs2=True, id_=vmid)
    assert await env.req.drive_request(port=port, vaddr=gpa_in, cmd=LOAD, return_on_miss=True) is None
...
async def test_hfence_gvma_by_gpa_vmid(dtlb_env):
    ...
    await do_sfence(env, hv=False, hg=True, rs1=True, addr=gpa_in, rs2=True, id_=vmid)
    assert await env.req.drive_request(port=port, vaddr=gpa_in, cmd=LOAD, return_on_miss=True) is None
