...
def init_rar_funcov(rarqueue, g: fc.CovGroup):
    ...
    g.mark_function("RAR_ENQUEUE_0", _M("test_update_queue"), bin_name=["can receive violation query"])
    g.mark_function("RAR_ENQUEUE_1", _M("test_update_queue"), bin_name=["can receive violation query"])
    g.mark_function("RAR_ENQUEUE_2", _M("test_update_queue"), bin_name=["can receive violation query"])
    # The End
    return None
...
class LoadQueueRAR(toffee.Bundle):
    ...
    def Enqueue(self, query, redirect, ldWbPtr):
        ...
        self.ldWbPtr["flag"].value = int(str(ldWbPtr)[0])
        self.ldWbPtr["value"].value = int(str(ldWbPtr)[1:8])
