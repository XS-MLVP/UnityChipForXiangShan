...
class LoadQueueRAWAgent(Agent):
    ...
    @driver_method()
    async def detect(self, storeIn: List[StoreIn]):
        for i in range(2):
            storeIn_i = getattr(self.bundle.io._storeIn, f'_{i}')
            storeIn_i._valid.value = storeIn[i].valid
            storeIn_i._bits._uop_robIdx._flag.value = storeIn[i].robIdx_flag
            storeIn_i._bits._uop_robIdx._value.value = storeIn[i].robIdx_value
        ...
        return rollback, self.bundle.LoadQueueRAW