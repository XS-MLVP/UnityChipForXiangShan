...
from ..util.dataclass import IOQuery, IOldWbPtr, IORedirect
...
@toffee_test.testcase
async def test_ctl_releasedupdate(
        loadqueue_rar_env: LoadQueueRAREnv):
    await loadqueue_rar_env.agent.reset()
    query = [
        IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=4,
                uop_lqIdx_flag=True, uop_lqIdx_value=4, bits_paddr=747113358170,
                data_valid=False, is_nc=True, revoke=False),

        IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=5,
                uop_lqIdx_flag=True, uop_lqIdx_value=6, bits_paddr=183041682239,
                data_valid=True, is_nc=False, revoke=False),

        IOQuery(req_valid=False, uop_robIdx_flag=True, uop_robIdx_value=3,
                uop_lqIdx_flag=True, uop_lqIdx_value=8, bits_paddr=773314057378,
                data_valid=True, is_nc=False, revoke=False),
    ]
...
def generate_random_48bit_integer():
    decimal_value = random.getrandbits(48)
    ...
    for i in range(23):
        query = [
            IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=i * 3, uop_lqIdx_flag=True,
                    uop_lqIdx_value=i * 3, bits_paddr=generate_random_48bit_integer(), data_valid=True, is_nc=False,
                    revoke=False),

            IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=i * 3 + 1, uop_lqIdx_flag=True,
                    uop_lqIdx_value=i * 3 + 1, bits_paddr=generate_random_48bit_integer(), data_valid=True, is_nc=False,
                    revoke=False),

            IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=i * 3 + 2, uop_lqIdx_flag=True,
                    uop_lqIdx_value=i * 3 + 2, bits_paddr=generate_random_48bit_integer(), data_valid=True, is_nc=False,
                    revoke=False),
        ]
...
async def test_freelist_full(loadqueue_rar_env: LoadQueueRAREnv):
    ...
    for i in range(24):
        query = [
            ...
            IOQuery(req_valid=True, uop_robIdx_flag=True, uop_robIdx_value=i * 3, uop_lqIdx_flag=True,
                    uop_lqIdx_value=i * 3 + 3, bits_paddr=generate_random_48bit_integer(), data_valid=True, is_nc=False,
                    revoke=False),
        ]
        redirect = IORedirect(valid=False, robIdx_flag=True, robIdx_value=0, level=0)
        ldWbPtr = IOldWbPtr(flag=True, value=0)
    ...
    for _ in range(5000):
        query = [
            IOQuery(
                ...
                uop_lqIdx_flag=not random.getrandbits(1),
                uop_lqIdx_value=random.randint(0, 71),
                bits_paddr=generate_random_48bit_integer(),
                data_valid=not random.getrandbits(1),
                is_nc=not random.getrandbits(1),
