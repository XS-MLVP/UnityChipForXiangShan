...
async def test_smoke(lsq_rar_env: LoadQueueRAREnv):
    random.seed(os.urandom(128))
    await lsq_rar_env.agent.reset()
    num_queries = 3
    for _ in range(10):
        query = [
            IOQuery(
        ...
        _ = await lsq_rar_env.agent.Enqueue(query, redirect, ldWbPtr)

    
@toffee_test.fixture
async def lsq_rar_env(toffee_request: toffee_test.ToffeeRequest):
    import asyncio
    dut = toffee_request.create_dut(
        DUTLoadQueueRAR, "clock")
    toffee.start_clock(dut)
    lsq_rar_env = LoadQueueRAREnv(dut)
    toffee_request.add_cov_groups([
        init_rar_funcov(lsq_rar_env),
        get_coverage_group_of_update_register(lsq_rar_env)
    ])
