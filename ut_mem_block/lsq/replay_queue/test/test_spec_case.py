...
async def test_smoke(loadqueue_replay_env: LoadQueueReplayEnv):
...
async def loadqueue_replay_env(toffee_request: toffee_test.ToffeeRequest):
    ...
    loadqueue_replay_env = LoadQueueReplayEnv(dut)
    toffee_request.add_cov_groups(init_replay_funcov(loadqueue_replay_env))
    
    yield loadqueue_replay_env
