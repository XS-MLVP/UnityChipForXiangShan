...
async def test_smoke(loadqueue_virtual_env: VirtualLoadQueueEnv):
...
async def loadqueue_virtual_env(toffee_request: toffee_test.ToffeeRequest):
    ...
    loadqueue_virtual_env = VirtualLoadQueueEnv(dut)
    toffee_request.add_cov_groups(init_virtual_funcov(loadqueue_virtual_env))
    
    yield loadqueue_virtual_env
